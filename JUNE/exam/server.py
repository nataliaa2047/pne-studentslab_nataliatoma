...
class GenomeRequestHandler(http.server.BaseHTTPRequestHandler):     #This class evaluates what a user typed in their browser, directing them to the correct execution block
    def do_GET(self):               #Every time a user requests a URL from this server, this do_GET method automatically runs
        ...
        elif self.path.startswith("/compare_genes"):
            g1_val = ""
            g2_val = ""
            if "g1=" in self.path:
                g1_val = self.path.split("g1=")[1].split("&")[0].strip()
            if "g2=" in self.path:
                g2_val = self.path.split("g2=")[1].split("&")[0].strip()
            if not g1_val or not g2_val:
                self.serve_error_file(
                    is_json_format,
                    title="ERROR",
                    message="Missing parameters: Please provide both Gene IDs (g1 and g2)."
                )
            else:
                self.handle_compare_genes(g1_val, g2_val, is_json_format)
    ...
    def handle_compare_genes(self, g1, g2, is_json):
        data1 = get_ensembl_data(f"/lookup/id/{g1}?expand=1")
        data2 = get_ensembl_data(f"/lookup/id/{g2}?expand=1")
        if data1 is None or "error" in data1 or "Transcript" not in data1:
            self.serve_error_file(is_json, title="Ensembl API Error (400)",
                                  message="Please, check if the gene IDs are correct.")
            return
        if data2 is None or "error" in data2 or "Transcript" not in data2:
            self.serve_error_file(is_json, title="Ensembl API Error (400)",
                                  message="Please, check if the gene IDs are correct.")
            return
        ...
        num_transcripts1 = len(transcripts1)

        max_exons1 = 0
        for t in transcripts1:
            exons1 = t.get("Exon", [])
            if len(exons1) > max_exons1:
                max_exons1 = len(exons1)

        name2 = data2.get("display_name", g2)
        transcripts2 = data2.get("Transcript", [])
        num_transcripts2 = len(transcripts2)

        max_exons2 = 0
        for t in transcripts2:
            exons2 = t.get("Exon", [])
            if len(exons2) > max_exons2:
                max_exons2 = len(exons2)

        if num_transcripts1 > num_transcripts2:
            diff = num_transcripts1 - num_transcripts2
            veredict = f"Veredict: {name1} is more complex, containing {diff} more transcript(s) than {name2}"
        elif num_transcripts2 > num_transcripts1:
            diff = num_transcripts2 - num_transcripts1
            veredict = f"Veredict: {name2} is more complex, containing {diff} more transcript(s) than {name1}"
        else:
            veredict = f"Veredict: Both genes are equally complex, containing {num_transcripts1} transcript(s) each"

        if is_json:
            self.send_json_response(200, {
                "veredict": veredict,
                "gene1": {"id": g1, "display_name": name1, "transcripts": transcripts1, "max_exons": max_exons1},
                "gene2": {"id": g2, "display_name": name2, "transcripts": transcripts2, "max_exons": max_exons2}
            })
            return

        html = f"""<html><body>
            <h1>Gene Comparison Report</h1>
            <h2>{veredict}</h2>
            <h3>Gene 1: {name1}</h3>
            <p><strong>Ensemble ID:</strong> {g1}</p>
            <p><strong>Total Transcripts:</strong> {num_transcripts1}</p>
            <p><strong>Max Exons (in a single transcript):</strong> {max_exons1} bp</p>
            <h3>Gene 2: {name2}</h3>
            <p><strong>Ensemble ID:</strong> {g2}</p>
            <p><strong>Total Transcripts:</strong> {num_transcripts2}</p>
            <p><strong>Max Exons (in a single transcript):</strong> {max_exons2} bp</p>
